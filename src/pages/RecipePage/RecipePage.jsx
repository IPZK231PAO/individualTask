...
const RecipePage = () => {
	...
	const info = useStore(state => state.idData)
	useEffect(() => {
		fetchByID(id)
	}, [fetchByID])
	console.log(info)
					...
					<div className={styles.info}>
						...
						<div>
							...
							<div className={styles.foodPairing}>
								<h3>Food pairing</h3>
								<ul>
									{info.food_pairing?.map((item, index) => (
										<li key={index}>
											<IoFastFoodSharp /> {item}
										</li>
									))}
								</ul>
							</div>
						...
						<div className={styles.recipe}>
							...
							<div className={styles.ingredients}>
								...
								<ul>
									...
									<ul className='malts-ul'>
										{' '}
										{info.ingredients?.malt.map((item, index) => (
											<li key={index}>
												<p>
							...
							<div className={styles.method}>
								...
								<ul>
									...
									<ul>
										<h4>Mesh temp:</h4>
										{info.method?.mash_temp.map((item, index) => (
											<li key={index}>
